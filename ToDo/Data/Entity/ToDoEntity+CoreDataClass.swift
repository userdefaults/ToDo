...
@objc(ToDoEntity)
public class ToDoEntity: NSManagedObject, Codable {
    public convenience required init(from decoder: any Decoder) throws {
        guard let context = decoder.userInfo[.moc] as? NSManagedObjectContext else {
            throw ToDoEntityError.noContextInDecoder
        }
...
extension ToDoEntity {
    ...
    enum ToDoEntityError: LocalizedError {
        case noContextInDecoder
        
        var errorDescription: String? {
            switch self {
            case .noContextInDecoder:
                "No context found in decoder"
            }
        }
        ...
        var failureReason: String? {
            switch self {
            case .noContextInDecoder:
                "No NSManagedObjectContext given to decoder"
            }
        }
...
extension ToDoEntity {
    static let mockData: [ToDoEntity] = {
        let toDoString = """
            [
                {
                    "id": 1,
        ...
        guard let data = toDoString.data(using: .utf8) else {
            return []
        }
        
        guard let result = try? decoder.decode([ToDoEntity].self, from: data) else {
